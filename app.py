...
Set your Anthropic API key in .env or Streamlit secrets:
...
.metric-card .val { font-size: 2rem; font-family: 'Cormorant Garamond', serif; font-weight: 700; color: var(--gold); }
.metric-card .lbl { font-size: 0.72rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); }
...
.chat-user   { background:#fff; border-left:3px solid var(--gold); padding:0.7rem 1rem; border-radius:0 4px 4px 0; margin-bottom:0.5rem; }
.chat-ai     { background:var(--mid); color:var(--cream); border-left:3px solid var(--muted); padding:0.7rem 1rem; border-radius:0 4px 4px 0; margin-bottom:0.5rem; }
.debrief-box { background:#fff; border:1px solid var(--gold-lt); border-radius:4px; padding:1.5rem; margin-top:1rem; }
...
TIER_CONFIG = {
    1: {"name": "Beginner",     "color": "#8C7A5A", "sessions_needed": 10, "avg_score_needed": 6},
    2: {"name": "Intermediate", "color": "#C9A84C", "sessions_needed": 15, "avg_score_needed": 7},
    3: {"name": "Expert",       "color": "#E8D5A3", "sessions_needed": 999,"avg_score_needed": 9},
}

POINTS = {"participation": 10, "growth_bonus": 20}

# ── Customer profiles (Phase 1: 3 profiles) ───────────────────────────────────
PROFILES = {
    1: {
        "id": 1,
...
# ── Minimal knowledge base (inject into prompt — expand before Phase 2) ───────
KNOWLEDGE_BASE = """
...
- Offer a brief, honest question before recommending: "Are you looking for something for yourself or as a gift?"
...
PRODUCT SAMPLE CATALOGUE
1. Al Majd (العظمة) — Oud & Rose
   Family: Oriental Woody | Notes: Agarwood / Taif Rose / Amber
   Concentration: Extrait | Sizes: 30ml, 50ml | Price: SAR 650 / SAR 950
...
2. Sahara Musk — White Musk & Sandalwood
   ...
   Concentration: EDP | Sizes: 50ml, 100ml | Price: SAR 380 / SAR 520
...
3. Darb Al Hind (درب الهند) — Spiced Oud
   ...
   Concentration: EDP Intense | Sizes: 50ml | Price: SAR 820
   Story: Named for the ancient incense trade route. Bold, complex, unforgettable.

OBJECTION HANDLING TEMPLATES
- "Too expensive": "This is an extrait — the concentration is twice that of a standard EDP.
  You're wearing it for 12–14 hours on one application. Per wear, it is more economical than it seems."
- "Not sure I like oud": "Let me show you Sahara Musk first — it is a soft introduction,
  no smoke, just warmth. Then we can travel further if you'd like."
- "Already have perfume": "Of course — a great fragrance collection needs variety.
  This one is for occasions when you want to be remembered."
...
def save_session(salesperson, profile_id, sale, svc, points, transcript, debrief):
    with engine.connect() as conn:
        conn.execute(text("""
            INSERT INTO sessions (salesperson, profile_id, sale_score, svc_score, points, transcript, debrief)
            VALUES (:sp, :pid, :sale, :svc, :pts, :tx, :db)
        """), dict(sp=salesperson, pid=profile_id, sale=sale, svc=svc, pts=points,
                   tx=json.dumps(transcript), db=debrief))
        conn.execute(text("""
...
def load_salespeople() -> pd.DataFrame:
    with engine.connect() as conn:
        df = pd.read_sql("SELECT * FROM salespeople", conn)
    return df
...
def build_persona_system(profile: dict) -> str:
...
OPENING LINE (use this to start if it's the first message): {profile['opening_line']}
EMOTIONAL TRIGGERS: {', '.join(profile['triggers'])}
TYPICAL OBJECTIONS: {chr(10).join('- ' + o for o in profile['objections'])}
...
- React authentically to good selling technique (warm up, open up).
- React to poor technique (go cold, disengage, object more).
...
10 Perfect needs discovery, compelling story, handled all objections, natural close, upsell attempted
8–9 Strong performance with minor gaps
...
10 Warm, patient, culturally appropriate, active listening, never rushed customer
8–9 Mostly warm with minor lapses
6–7 Generally polite but mechanical or slightly rushed
4–5 Transactional — no genuine connection
...
def chat_as_customer(profile: dict, messages: list) -> str:
    ...
    response = client.messages.create(
        model=MODEL,
        max_tokens=300,
        system=build_persona_system(profile),
        messages=messages,
    )
...
def score_session(transcript: list) -> dict:
    ...
    response = client.messages.create(
        model=MODEL,
        max_tokens=1000,
        system=build_scoring_system(),
        messages=[{"role": "user", "content": f"Please score this session:\n\n{transcript_text}"}],
    )
...
def build_debrief(scores: dict, transcript: list, sp_name: str,
                  prev_sale_avg: float, prev_svc_avg: float, points_earned: int) -> str:
    sale = scores["sale_score"]
    svc  = scores["service_score"]
    growth_msg = ""
    if prev_sale_avg > 0 and sale > prev_sale_avg:
        growth_msg = f"Your Sale score of {sale} is above your previous average of {prev_sale_avg:.1f} — that's real growth. "
...
◎ POINTS EARNED THIS SESSION
  +10 Participation
  +{max(0, int((sale + svc) * 5))} Performance  ({sale + svc}/20 combined)
  {''+20+' Growth bonus — you improved on your last session!' if '20' in str(points_earned) else ''}
  Total: {points_earned} points

◈ NEXT RECOMMENDED SESSION
  {scores['next_session_recommendation']}

Keep going. The best salespeople are not born — they are made, one session at a time. You are on your way.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
""".strip()
...
def render_sidebar():
    with st.sidebar:
        ...
        page = st.radio("Navigate", ["Practice Session", "My Progress", "Manager Dashboard"],
                        label_visibility="collapsed")
        st.divider()

        st.markdown("**Salesperson**")
        sp_name = st.text_input("Your name", value=ss_get("sp_name", ""), key="sp_input",
        ...
        tier = ss_get("tier", 1)
        tier_label = TIER_CONFIG[tier]["name"]
        st.markdown(f'<span class="badge">Tier {tier} · {tier_label}</span>', unsafe_allow_html=True)
        pts = ss_get("total_points", 0)
        st.markdown(f'<span class="badge">⭐ {pts} pts</span>', unsafe_allow_html=True)
...
def page_practice():
    ...
    sp_name = ss_get("sp_name", "")
    if not sp_name:
        st.info("👈 Enter your name in the sidebar to begin.")
        return

    # Profile selection
    if not ss_get("session_active"):
        ...
        for i, (pid, p) in enumerate(PROFILES.items()):
            with cols[i]:
                ...
                if st.button(f"Practice with {p['name']}", key=f"pick_{pid}"):
    ...
    profile = PROFILES[ss_get("active_profile")]
    st.markdown(f"### {profile['emoji']} Session — {profile['name']}")
    st.caption(profile["brief"])
    st.divider()

    # Debrief view
    if ss_get("debrief_done"):
        debrief_text = ss_get("last_debrief", "")
        st.markdown('<div class="debrief-box">', unsafe_allow_html=True)
        st.text(debrief_text)
    ...
    # Chat
    msgs = ss_get("messages", [])

    # Auto-open from customer
    if not msgs:
        with st.spinner("Customer entering the store…"):
            opening = chat_as_customer(profile, [
                {"role": "user", "content": "[Session starts. You are now in the store. Deliver your opening line.]"}
            ])
    ...
    with chat_container:
        for m in msgs:
            if m["role"] == "user":
                st.markdown(f'<div class="chat-user">🧑 <strong>You:</strong> {m["content"]}</div>',
                            unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="chat-ai">👤 <strong>Customer:</strong> {m["content"]}</div>',
                            unsafe_allow_html=True)

    col_input, col_end = st.columns([5, 1])
    with col_input:
        user_input = st.text_input("Your response", key="chat_input", placeholder="What do you say to the customer?",
                                   label_visibility="collapsed")
    with col_end:
        end_clicked = st.button("End & Score", type="primary")

    if user_input and st.session_state.get("chat_input"):
    ...
    if end_clicked and len(msgs) >= 3:
        with st.spinner("Evaluating your session…"):
            ...
            prev_sale = df["sale_score"].mean() if not df.empty else 0
            prev_svc  = df["svc_score"].mean()  if not df.empty else 0
            growth = 20 if (scores["sale_score"] > prev_sale and prev_sale > 0) else 0
            total_pts = POINTS["participation"] + perf_pts + growth

            debrief = build_debrief(scores, msgs, sp_name, prev_sale, prev_svc, total_pts)
            ...
            ss_set("total_points", ss_get("total_points", 0) + total_pts)
...
def page_dashboard():
    ...
    with col_a:
        ...
        lb = all_df.groupby("salesperson").agg(
            Sessions=("id","count"),
    ...
    trend = all_df.groupby(all_df["created_at"].dt.date).agg(
        Sale=("sale_score","mean"), Service=("svc_score","mean")
    ).reset_index()
    fig3 = px.line(trend, x="created_at", y=["Sale","Service"],
                   color_discrete_map={"Sale":"#C9A84C","Service":"#8C7A5A"})
    fig3.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                       font=dict(family="Jost"), xaxis_title="", yaxis=dict(range=[0,10]))
